...
public class EmpregadoRest {
    ...
    @Path("{matricula}")
    @Produces(MediaType.APPLICATION_JSON)
    public Response obterEmpregado(@PathParam("matricula") String matricula) {
        try {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    public Response excluirEmpregado(@PathParam("matricula") String matricula) {
        try {
            if (Banco.buscarEmpregadoPorMatricula(matricula) == null) {
                return Response.status(Response.Status.NOT_FOUND)
                        .entity("{ \"mensagem\" : \"Empregado nao encontrado!\" }").build();
            ...
            return Response.ok()
                    .entity("{ \"mensagem\" : \"Empregado " + matricula + " excluido!\" }").build();
        } catch (Exception e) {
            return Response.status(Response.Status.INTERNAL_SERVER_ERROR)
